...
class WorkflowUnit(baseWorkflowUnit.BaseWorkflowUnit):
    ...
    def __init__(self, GUI, unitID):
        ...
        self.convertionTypes = {
            "tif": ["SMALL", "MEDIUM", "LARGE"],
    ...
    def run(self, unitWorkFlowVarScope):
        ...
        takeFromAccess = self.getSetupValue(
            "take_from_access", isRequired=True
        )
        ...
        self.largeThreshold = self.getSetupValue(
            "largeThreshold", isRequired=True
        )
        self.mediumThreshold = self.getSetupValue(
            "mediumThreshold", isRequired=True
        )
        self.smallThreshold = self.getSetupValue(
            "smallThreshold", isRequired=True
        )
        ...
        self.mediumPathDir = self.getSetupValue(
            "mediumPathDir", isRequired=True
        )
        self.smallPathDir = self.getSetupValue("smallPathDir", isRequired=True)

        self.imageMagickFolder = self.getSetupValue(
            "imageMagick_folder", isRequired=True
        )
        self.imageMagickBinary = self.getSetupValue(
            "imageMagick_binary", isRequired=True
        )

        programPath = os.path.join(
            self.imageMagickFolder, self.imageMagickBinary
        )
        if not os.path.isfile(programPath):
            self.logMessage(
                "Missing imagemagick binary used for convertion!, %s not found..."
                % (programPath),
                CRITICAL_MSG,
            )

        if convertType == "PDF":
            self.ghostScriptFolder = self.getSetupValue(
                "ghostScript_folder", isRequired=True
            )
            self.ghostScriptBinary = self.getSetupValue(
                "ghostScript_binary", isRequired=True
            )

            programPath = os.path.join(
                self.ghostScriptFolder, self.ghostScriptBinary
            )
            if not os.path.isfile(programPath):
                self.logMessage(
                    "Missing ghostscript binary used for convertion!, %s not found..."
                    % (programPath),
                    CRITICAL_MSG,
                )
        ...
        try:
            ...
            self.logMessage(
                'CSV header: "%s", does not contain the YAML specified column "%s" used in key:oasid_col'
                % (header, oasIDCol),
                CRITICAL_MSG,
            )
        ...
        try:
            ...
            self.logMessage(
                'CSV header: "%s", does not contain the YAML specified column "%s" used in key:oas_dict_key_filename'
                % (header, colDictColName),
                CRITICAL_MSG,
            )
        ...
        for i, row in enumerate(data):
            ...
            if legal_restrictions and int(legal_restrictions["id"]) > 1:
                self.logMessage(
                    "-Skipped: file restricted by privacy-settings", STATUS_MSG
                )
                continue
            ...
            if not takeFromAccess:
                ...
                if convertType == "PDF":
                    ...
                    if not os.path.isfile(fullFileName):
                        self.logMessage(
                            "-Done Skipped no access PDF", STATUS_MSG
                        )
                        continue
                    ...
                    if not os.path.isfile(fullFileName):
                        self.logMessage(
                            "-Done Skipped no access JPG", STATUS_MSG
                        )
                        continue
            ...
            if not os.path.isfile(fullFileName):
                self.logMessage(
                    "%s - No binary file pressent with path: %s"
                    % (oasID, fullFileName),
                    WARNING_MSG,
                )
                continue
            ...
            if len(convertionList) == 0:
                self.logMessage(
                    "-Skipped: file type is not handled by this module",
                    STATUS_MSG,
                )
                continue

            distFilenameLarge = os.path.join(
                self.largePathDir, ".".join(("%s_l" % oasID, self.webFormat))
            )
            distFilenameMedium = os.path.join(
                self.mediumPathDir, ".".join(("%s_m" % oasID, self.webFormat))
            )
            distFilenameSmall = os.path.join(
                self.smallPathDir, ".".join(("%s_s" % oasID, self.webFormat))
            )

            allowToSkip = True
            if overwriteBoolean:
                allowToSkip = False
            elif "LARGE" in convertionList and not os.path.isfile(
                distFilenameLarge
            ):
                allowToSkip = False
            elif "MEDIUM" in convertionList and not os.path.isfile(
                distFilenameMedium
            ):
                allowToSkip = False
            elif "SMALL" in convertionList and not os.path.isfile(
                distFilenameSmall
            ):
                allowToSkip = False
            ...
            if allowToSkip:
                self.logMessage(
                    "-Skipped: file already has binary access versions",
                    STATUS_MSG,
                )
                continue
            ...
            if convertType == "IMAGE":
                ...
                if fileType == "tif":
                    fullFileName = (
                        fullFileName + "[0]"
                    )  ##fix to not make multi images out of multi layered tif
            else:  # convertType == "PDF"
                self.logMessage("-Pdf", STATUS_MSG)
                formatConvert = "tif"

                tempFilePath = (
                    self.GUI.WORKFLOW_PATH
                    + os.path.sep
                    + "temp."
                    + formatConvert
                )
    ...
    def getImagePixelSize(self, srcFileName):
        try:
            ...
            p = subprocess.Popen(
                argList, startupinfo=startupinfo, stdout=subprocess.PIPE
            )
