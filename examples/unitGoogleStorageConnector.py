...
def create_service(ca_certs):
    ...
    return googleapiclient.discovery.build(
        "storage", "v1", http=http, credentials=credentials
    )
...
class WorkflowUnit(baseWorkflowUnit.BaseWorkflowUnit):
    ...
    def __init__(self, GUI, unitID):
        ...
        self.resourceDir = os.path.join(
            GUI.WORKFLOW_PATH, "resources", "googleUpload"
        )
    ...
    def run(self, unitWorkFlowVarScope):
        ...
        googleCredentials = self.getSetupValue(
            "google_credentials", isRequired=True
        )

        googleCredentialsPath = os.path.join(
            self.resourceDir, googleCredentials
        )
        if not os.path.isfile(googleCredentialsPath):
            self.logMessage(
                "missing google service account json file %s"
        ...
        if not isABatchJob:
            if operation == "UPLOAD":
                if not destFile:
                    self.logMessage(
                        "no valid csv save file have been choosen",
                        CRITICAL_MSG,
                    )
                ...
                service = create_service(
                    os.path.join(self.resourceDir, "cacerts.txt")
                )
                    ...
                    if len(item) == 2:
                        ...
                        if os.path.isfile(filePath):
                            ...
                            if recordType == "image":
                                ...
                                self.logMessage(
                                    "record %s with record_type %s is not supported"
                ...
                if activeItem:
                    self.logMessage(
                        "DONE %s-%s" % (progress, nrOfRecords), STATUS_MSG
                    )
                ...
                for key, valueDict in sorted(
                    uploaded.items(), key=lambda e: e[0]
                ):
                    newRow = []
